...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    _applyView() {
      ...
      // Default-crop anchor: when the user hasn't reframed (view at 0,0), honor
      // the `position` attribute as the cover anchor — e.g. "50% 12%" biases the
      // crop toward the top so a portrait's head isn't cut off.
      let vx = this._view.x, vy = this._view.y;
      if (vx === 0 && vy === 0) {
        const pos = this.getAttribute('position');
        if (pos) {
          const pp = pos.split(/\s+/);
          const px = parseFloat(pp[0]), py = parseFloat(pp[1]);
          const mx = Math.max(0, (g.iw * k / g.fw - 1) * 50);
          const my = Math.max(0, (g.ih * k / g.fh - 1) * 50);
          if (!isNaN(px)) vx = (50 - px) / 50 * mx;
          if (!isNaN(py)) vy = (50 - py) / 50 * my;
        }
      }
      const l = (50 + vx) + '%';
      const t = (50 + vy) + '%';
